...
double DiscreteFrechet::recursiveFunc(TimeSeries *Ts1,size_t n1,TimeSeries *Ts2, size_t n2,size_t dim, double *ca,size_t idx1 ,size_t idx2)
{
    double output;
    ...
    else if ( idx1 == 0 && idx2 == 0)
    {
        ...
        for ( i = 0; i < dim; i++)
        {
            tmp2 = timeSerieDataFirstDouble[idx1 + n1 * i] - timeSerieDataSecondDouble[idx2 + n2 * i];
            tmp1 +=  tmp2 * tmp2;
        }
...
void DiscreteFrechet::setParam()
{
    char answer;
    string winSize;
    cout << "Do you Want Enter winSize y/n : ";
    cin >> answer;
    if(answer == 'y' || answer=='Y')
    {
        ...
        winSize = "-1";
    if(isInteger(winSize))
        this->winSize = stoi(winSize);
}

//-------------------------------------------------------------------------------------------------

size_t DiscreteFrechet::getParam()
{
    return winSize;
}
