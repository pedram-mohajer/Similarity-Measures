...
void Erp::compute(TimeSeries *Ts1, TimeSeries *Ts2 ,double & _erpDist,int &_erpLen )
{
    ...
    for (i = 0; i <= n1; i++)
    {
        for (j = 0; j <= n2; j++)
        {
            ...
            if (i == 0 && j >= 1)
            {
                D[0][j] = D[0][j-1] + euclideanCalculate(timeSerieDataSecondDouble, n2, gap, 1, dim1, j-1, 0);
            }
            if (j == 0 && i>= 1)
            {
                D[i][0] = D[i-1][0] + euclideanCalculate(timeSerieDataFirstDouble, n1, gap, 1, dim1, i-1, 0);
            }
        }
    }
    ...
    for (i = 1; i <= n1; i++)
    {
        ...
        for (j = jS; j <= jF; j++)
        {
            D[i][j] = D[i-1][j-1] + euclideanCalculate(timeSerieDataFirstDouble, n1, timeSerieDataSecondDouble, n2, dim1, i-1, j-1);
            
            tmp = D[i][j-1] + euclideanCalculate(timeSerieDataSecondDouble, n2, gap, 1, dim1, j-1, 0);
            if (tmp < D[i][j])
            {
                D[i][j] = tmp;
            }
            tmp = D[i-1][j] + euclideanCalculate(timeSerieDataFirstDouble, n1, gap, 1, dim1, i-1, 0);
            if (tmp < D[i][j])
            {
                D[i][j] = tmp;
            }
...
Erp::Erp(int size,double * _winSize ,double * _winRatio,double * _gap)
{
    ...
    for(int i=0 ; _gap[i] != -1 ; i++ )
    	gap.push_back(_gap[i]);
...
//-------------------------------------------------------------------------------------------------
double Erp::euclideanCalculate(vector <double> ts1,int n1,vector <double>ts2,int n2,int dim, int idx1,int idx2)
{
    double euclDist = 0;
    int i;
    double tmp;
