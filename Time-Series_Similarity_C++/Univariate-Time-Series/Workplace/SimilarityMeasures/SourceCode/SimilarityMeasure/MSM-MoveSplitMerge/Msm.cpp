...
//-------------------------------------------------------------------------------------------------
double Msm::msmCost(vector <double> tsA, int nA, vector <double> tsB, int nB, vector <double> tsC, int nC, int dim, double cost, int idxA, int idxB, int idxC)
{
...
void Msm::compute(TimeSeries *Ts1, TimeSeries *Ts2 ,double & _msmDist,int &_msmLen )
{
    ...
    D[0][0] = euclideanCalculate(timeSerieDataFirstDouble, n1, timeSerieDataSecondDouble, n2, dim1, 0, 0);
    
    for (i = 1; i < n1; i++)
    {
        D[i][0] = D[i-1][0] + msmCost(timeSerieDataFirstDouble, n1, timeSerieDataFirstDouble, n1, timeSerieDataSecondDouble, n2, dim1, cost, i, i-1, 0);
    }
    
    for (j = 1; j < n2; j++)
    {
        D[0][j] = D[0][j-1] + msmCost(timeSerieDataSecondDouble, n2, timeSerieDataFirstDouble, n1, timeSerieDataSecondDouble, n2, dim1, cost, j, 0, j-1);
    }
    ...
    for (i = 1; i < n1; i++)
    {
        ...
        for (j = jS; j < jF; j++)
        {
            tmp = euclideanCalculate(timeSerieDataFirstDouble, n1, timeSerieDataSecondDouble, n2, dim1, i, j);
            D[i][j] = D[i-1][j-1] + tmp;
            
            tmp = D[i-1][j] + msmCost(timeSerieDataFirstDouble, n1, timeSerieDataFirstDouble, n1, timeSerieDataSecondDouble, n2, dim1, cost, i, i-1, j);
            ...
            tmp = D[i][j-1] + msmCost(timeSerieDataSecondDouble, n2, timeSerieDataFirstDouble, n1, timeSerieDataSecondDouble, n2, dim1, cost, j, i, j-1);
...
double Msm::euclideanCalculate(vector <double> ts1,int n1,vector <double> ts2,int n2,int dim, int idx1,int idx2)
{
    double euclDist = 0;
    int i;
    double tmp;
