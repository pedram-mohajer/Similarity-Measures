...
{
	vector <double> timeSeriesDouble (timeSeriesString.size() );
...
void validateTimeSeriesData(string filePath)
{
    ...
    size_t vector_size;
    int j=0;
    int fieldCount=0,sizeCount=0,COUNT=0,typeVariableCount=0,sampleCount=0;
    while(++j<=7)
    {
        getline(iFile, data);
        ...
        switch(j)
        {
            ...
            case 1 :
            {
                ...
                {
                    iFile.close();
                    throw ErrorInTsdHeaderFile("number of Data in VERSION line is invaild ",filePath);
                }
                if(token[0]!="VERSION")
                {
                    iFile.close();
    ...
    int counter=-1; // For Counting number of Data
    ...
    while(iFile)
    {
        ...
        {
            iFile.close();
            throw ErrorInTsdFooterFile("Data Type is Invaild " ,filePath);
        }
        counter++;
    }
    ...
    {
        iFile.close();
        throw ErrorInTsdFooterFile("Number Of Sample is Invaild ",filePath);
    }
    iFile.close();
