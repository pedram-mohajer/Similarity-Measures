...
void Cidtw::compute(TimeSeries *Ts1, TimeSeries *Ts2 ,double & _cidtwDist,int &_cidtwLen )
{
    ...
    vector <string>  timeSerieDataFirstStr   = Ts1->getTimeSerieData();
    vector <string>  timeSerieDataSecondStr  = Ts2->getTimeSerieData();


    vector <double> timeSerieDataFirst  =  VectorToDouble(timeSerieDataFirstStr);
    vector <double> timeSerieDataSecond =  VectorToDouble(timeSerieDataSecondStr);
    ...
    for ( i = 0; i < n1 - 1; i++)
    {
        for (j = 0; j <dim1; j++)
        {
            tmp = timeSerieDataFirst[i + n1 * j] - timeSerieDataFirst[ (i+1) + n1 * j];
    ...
    for ( i = 0; i < n2 - 1; i++)
    {
        for (j = 0; j <dim1; j++)
        {
            tmp = timeSerieDataSecond[i + n2 * j] - timeSerieDataSecond[ (i+1) + n2 * j];
            CE2 += tmp * tmp;
        }
    }
...
double Cidtw::euclideanCalculate(vector <double> ts1,int n1,vector <double> ts2,int n2,int dim, int idx1,int idx2)
{
    double euclDist = 0;
    int i;
    double tmp;
