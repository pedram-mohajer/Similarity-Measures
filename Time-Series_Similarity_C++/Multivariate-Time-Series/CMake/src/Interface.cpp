 ...
 void  Interface::chooseMethod(vector <string>  path,vector <double * > param,int userChoiceNumber,string userChoiceStr)
{
	...
	size_t Dimension	 =  (size_t)*param[16];
	size_t DatasetNumber =  (size_t)*param[14];


    readDataset(path[0],DatasetNumber);
    checkTimeSeriesData();
	createTimeSeriesInstances(Dimension);
    ...
    {
    	readLablesPath(path[1], 1);
    	setTimeSeriesLabels(1);
...
//------------------------------------------------------------------------------------------------
unique_ptr <TimeSeries> Interface::buildTimeSeries( string  path , size_t Dim)
{
    unique_ptr <TimeSeries> time(new TimeSeries(path, Dim));

    return time;
}

//------------------------------------------------------------------------------------------------
Interface::Interface(int datasetSize)
{
	dataPathRepository.resize(datasetSize);
	timeSeries.resize(datasetSize);
	resultRepository = new ResultRepository(datasetSize);
    labelRespository.resize(datasetSize);
}
...
//------------------------------------------------------------------------------------------------
 ...
 Interface * Interface::makeInterfaceInstance(int datasetSize)
{
...
void Interface::createTimeSeriesInstances(size_t Dim)
{
	size_t size = dataPathRepository.size();
	...
	for( size_t i=0 ; i<size ; i++)
		for( size_t j=0 ; j<dataPathRepository[i].size() ; j++)
			this->timeSeries[i].push_back( buildTimeSeries( dataPathRepository[i][j] ,  Dim) );
...
void Interface::setTimeSeriesLabels(size_t numberOfDataset)
{
	...
	for(int i=0 ; i<dataPathRepository.size() ; i++)
			if(dataPathRepository[i].size() != lables[i].size() )
				throw(InequalityInNumberOFLabelsAndData(string(string("Inequality In Number Of Labels And Data In ") + labelPathRepository[i]) , "Interface.cpp"));
