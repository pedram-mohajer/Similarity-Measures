...
Facade::Facade() : userChoiceInt(-1), methodNumber(-1) , numberOfTimeseriesInt(-1)
{
...
void Facade::setParam(vector <string> input)
{
    ...
    setWinSize(input[1]);						//	param[0]
    setWinRatio(input[2]);						//	param[1]
    setEpsilon(input[3]);						//	param[2]
    setCost(input[3]);							//	param[3]
    setLambda( input[4]);						//	param[4]
    setNu(input[6]);							//	param[5]
    setEpc(input[10]);							//	param[6]
    setGapc(input[11]);							//	param[7]
    setReward(input[12]);						//	param[8]
    setGap(input[13]);							//	param[9]
    setWeightMax(input[14]);					//	param[10]
    setG(input[15]);							//	param[11]
    setFoldNumber(input[17]);					//	param[12]
    setIterNumber(input[18]);					//	param[13]
    setNumberOfDataset(input[7]);				//	param[14]
    setClassiferNumber(input[20]);				//	param[15]
    setNumberOfDimension(input[21]);			//	param[16]

}
//----------------------------------------------------------------------------------------------------
void Facade::createInterface()
{
    unique_ptr <Interface> interface(Interface::makeInterfaceInstance( (int)*param[14] ));
...
void Facade::setWinRatio(string _winRatio)
{
    if( !isNumber(_winRatio) && _winRatio.compare("NULL") )
        throw WrongInput("winRatio Number is not Valid","Facade.cpp");
...
//---------------------------------------------------------------------------------------------------
void Facade::setNumberOfDimension(string _NumberOfDimension)
{

    if( !isInteger(_NumberOfDimension) )
        throw WrongInput("Dimension is not Valid","Facade.cpp");
    if( isNumber(_NumberOfDimension) )
    {
        double *tmp = new double [1];
        tmp[0] = stod(_NumberOfDimension) ;
        param.push_back(tmp);
    }
    else
       param.push_back(NULL);
...
void Facade::setNumberOfDataset(string _classifierNumber)
{
    if( !isNumber(_classifierNumber) && _classifierNumber.compare("NULL") )
        throw WrongInput("classifierNumber is not Number");
    if( isNumber(_classifierNumber) )
