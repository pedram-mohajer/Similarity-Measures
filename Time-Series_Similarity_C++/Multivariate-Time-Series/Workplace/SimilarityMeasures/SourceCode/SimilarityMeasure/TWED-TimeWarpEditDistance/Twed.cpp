    ...
    for (i = 1; i <= n1; i++)
    {
        ...
        for (j = jS; j <= jF; j++)
        {
            if (i > 0 && j > 0)
            {
                ...
                tmp1 = euclideanCalculate(timeSerieDataFirst, n1, timeSerieDataSecond, n2, dim1, i - 2, j - 2);
            ...
            if (i > 0)
            {
                tmp = euclideanCalculate(timeSerieDataFirst, n1, timeSerieDataFirst, n1, dim1, i-1, i-2);
                tmpMin = D[i-1][j] + nu + lambda + tmp;
            }
            else
            {
                tmp = euclideanCalculate(timeSerieDataFirst, n1, Z , 1, dim1, i-1, 0);
                tmpMin = D[i-1][j] + nu + lambda + tmp;
            }
            if ( tmpMin < D[i][j] )
            {
            ...
            if (j > 0)
            {
                tmp = euclideanCalculate(timeSerieDataSecond, n2, timeSerieDataSecond, n2, dim1, j-1, j-2);
                tmpMin = D[i][j-1] + nu + lambda + tmp;
            }
            else
            {
                tmp = euclideanCalculate(timeSerieDataSecond, n2, Z, 1, dim1, j-1, 0);
                tmpMin = D[i][j-1] + nu + lambda + tmp;
            }
            if ( tmpMin < D[i][j] )
            {
...
double Twed::euclideanCalculate(vector<vector <string>> &ts1,int n1,vector<vector <string>> &ts2,int n2,int dim, int idx1,int idx2)
{
    double euclDist = 0;
    int i;
    double tmp;
