...
double Frechet::point2SegmentDistance(vector<vector <string>> &ts1, int n1, vector<vector <string>> &ts2, int n2, int dim, int idx1, int idx2)
{
...
void Frechet::criticValCalculate(vector<vector <string>> &ts1, int n1, vector<vector <string>> &ts2, int n2, int dim, vector<double> criticVal, int *criticSize)
{
    int i, j;
    ...
    for (i = 0; i < dim; i++)
    {
        tmp = stod(ts1[i][0])- stod(ts2[i][0]);
        tmp1 += tmp * tmp;
        tmp = stod(ts1[i][n1 - 1 + n1 ])- stod(ts2[i][n2 - 1 + n2]);
        tmp2 += tmp * tmp;
    }
...
//-------------------------------------------------------------------------------------------------
void Frechet::circleLineIntersectCalculation(vector<vector <string>> &ts1, int n1, vector<vector <string>> &ts2, int n2, int dim, double radius, int idx1, int idx2, double * intersect[] , int _column)
{
    ...
    xCenter = stod(ts1[0][idx1]);
    yCenter = stod(ts1[1][idx1]);
    lp1x = stod(ts2[0][idx2]);
    lp1y = stod(ts2[1][idx2]);
    lp2x = stod(ts2[0][idx2+1]);
    lp2y = stod(ts2[1][idx2+1]);
...
//-------------------------------------------------------------------------------------------------
void Frechet::freelineCalculate(vector<vector <string>> &ts1, int n1, vector<vector <string>> &ts2, int n2, int dim, double eps, int idx1, int idx2, double *valPt1, double *valPt2)
{
    double px, py;
    ...
    px =    stod(ts1[0][idx1]);
    py =    stod(ts1[1][idx1]);
    seg1x = stod(ts2[0][idx2]);
    seg1y = stod(ts2[1][idx2]);
    seg2x = stod(ts2[0][idx2+1]);
    seg2y = stod(ts2[1][idx2]);
...
//-------------------------------------------------------------------------------------------------
void Frechet::freespaceCalculate(vector<vector <string>> &ts1, int n1, vector<vector <string>> &ts2, int n2, int dim, double eps, double ***LF, double ***BF)
{
...
//-------------------------------------------------------------------------------------------------
bool Frechet::reachableCalculate(vector<vector <string>> &ts1, int n1, vector<vector <string>> &ts2, int n2, int dim, double eps)
{
...
void Frechet::compute(TimeSeries *Ts1, TimeSeries *Ts2 ,double & _frechetDist,int &_frechetLen )
{
    ...
    vector<double> criticVal(n1 * n2 * 2);
    ...
    criticValCalculate(timeSerieDataFirst, n1, timeSerieDataSecond, n2, dim1, criticVal, &criticSize);
    ...
    while (first <= last)
    {
        ...
        reach = reachableCalculate(timeSerieDataFirst, n1, timeSerieDataSecond, n2, dim1, eps);
